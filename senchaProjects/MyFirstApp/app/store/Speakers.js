Ext.define('MyFirstApp.util.Proxy', {
    singleton: true,
                ...
                Ext.Array.each(data.proposals, function(proposal) {
                    Ext.Array.each(proposal.speakers, function(speaker) {
                        ...
                        if (speakerIds.indexOf(speaker.id) == -1 && speaker.photo && speakerIds.length < 25) {
                            ...
                            speakerModel = Ext.create('MyFirstApp.model.Speaker', speaker);
...
Ext.define('MyFirstApp.store.Speakers', {
    extend: 'Ext.data.Store',

    config: {
        model: 'MyFirstApp.model.Speaker'
    }
});